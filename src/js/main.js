 ...
 * 動画タイムライン機能
 */
function initializeVideoTimeline() {
    const video = document.getElementById('interviewVideo');
    const timelineItems = document.querySelectorAll('.interview__timeline-item[data-time]');
    
    if (!video || timelineItems.length === 0) return;
    
    // タイムライン項目がクリックされたときの処理
    timelineItems.forEach(item => {
        ...
        item.addEventListener('mouseenter', function() {
            const marker = this.querySelector('.interview__timeline-marker');
            if (marker) {
    ...
    video.addEventListener('timeupdate', function() {
        ...
        timelineItems.forEach(item => {
            ...
            if (seconds !== null && Math.abs(currentTime - seconds) < 2) {
...
function highlightTimelineItem(item) {
    // 他の項目のハイライトを削除
    document.querySelectorAll('.interview__timeline-item').forEach(otherItem => {
        removeTimelineHighlight(otherItem);
    });
    
    // 現在の項目をハイライト
    item.classList.add('active');
    const marker = item.querySelector('.interview__timeline-marker');
    if (marker) {
        marker.style.background = 'var(--accent-color-2)';
 ...
 * タイムライン項目のハイライトを削除
 */
function removeTimelineHighlight(item) {
